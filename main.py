...
def ride(message):
    ...
    if len(parsed_message) == 1:
        ...
        if len(rides) == 0:
            return tb.send_message(chat_id, text=f"Não há ofertas de carona de {direction} :(")

        return tb.send_message(chat_id, str(rides), parse_mode='HTML')
    
    if not 3 < len(parsed_message) < 6:
        return tb.send_message(chat_id, text=f"Uso: {command} [horario] [vagas] [local]\n\
Ex: {command} 8:00 3 Barra Shopping")
    ...
    return_message = schedule_ride(chat_id, username, direction, time, available_seats, place)
...
def remove_ride(message):
    ...
    if len(parsed_message) != 2:
        return tb.send_message(chat_id, text='Uso: /remover [ida|volta]\nEx: /remover ida')
    ...
    session.query(Carpool).filter_by(chat_id=str(chat_id), telegram_username=username, carpool_type=carpool_type).delete()
    
    return tb.send_message(chat_id, text=f'@{username} removeu a carona de {direction}')
...
def update_available_seats(message):
    ...
    ride = session.query(Carpool).filter_by(chat_id=str(chat_id), telegram_username=username, carpool_type=direction).first()
    ...
    return tb.send_message(chat_id, text=f'@{username} atualizou o número de vagas para {available_seats}')
...
def set_ride_to_full(message):
    ...
    ride = session.query(Carpool).filter_by(chat_id=str(chat_id), telegram_username=username, carpool_type=carpool_type).first()
