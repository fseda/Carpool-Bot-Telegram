...
def get_rides_by_dir(chat_id, direction):
  ...
  rides = session.query(Carpool).filter_by(chat_id=str(chat_id),
...
def get_all_rides(chat_id):
  ...
  rides_going = session.query(Carpool).filter_by(chat_id=str(chat_id),
                                                 carpool_type='going',
                                                 is_active=True)
  yesterday_rides_going, today_rides_going, tomorrow_rides_going = separate_rides(
    rides_going)

  rides_returning = session.query(Carpool).filter_by(chat_id=str(chat_id),
                                                     carpool_type='returning',
                                                     is_active=True)
  yesterday_rides_returning, today_rides_returning, tomorrow_rides_returning = separate_rides(
    rides_returning)
...
def separate_rides(
    rides: list[Carpool]
) -> tuple[list[Carpool], list[Carpool], list[Carpool]]:
  today = datetime.now()
  yesterday = today - timedelta(days=1)
  tomorrow = today + timedelta(days=1)
...
def format_time(input):
  if ':' in input:
    ...
    return datetime(2020, 1, 1, hour=int(hour),
                    minute=int(minute)).strftime('%H:%M')
...
def set_rides_inactive(chat_id):
  ...
  rides = session.query(Carpool).filter(
    Carpool.chat_id == str(chat_id), Carpool.is_active == True,
